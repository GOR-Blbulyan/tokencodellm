#!/usr/bin/env python3
"""TokenCode AI v6.3: Local Torch + HuggingFace Router Qwen fallback chat."""
...
from utils.knowledge import CorpusDB, external_search, fetch_hf_rows_as_texts
...
HF_DATASET_URLS = [
    "https://datasets-server.huggingface.co/rows?dataset=google%2FMapTrace&config=default&split=train&offset=0&length=100",
    "https://datasets-server.huggingface.co/rows?dataset=openbmb%2FUltraData-Math&config=UltraData-Math-L1&split=train&offset=0&length=100",
    "https://datasets-server.huggingface.co/rows?dataset=GD-ML%2FIntTravel_dataset&config=default&split=train&offset=0&length=100",
    "https://datasets-server.huggingface.co/rows?dataset=OpenMed%2FMedical-Reasoning-SFT-Mega&config=default&split=train&offset=0&length=100",
    "https://datasets-server.huggingface.co/rows?dataset=HuggingFaceFW%2Ffineweb&config=default&split=train&offset=0&length=100",
]
...
def load_local_env_key():
    """Load HF_TOKEN from local ignored files if env var is absent."""
...
def ensure_ml_runtime(silent: bool = False) -> bool:
    ...
    if TORCH_IMPORT_ERROR is not None:
        if (not silent) and (not ML_RUNTIME_STATUS_PRINTED):
...
def generate_with_local_model(prompt: str, args) -> str | None:
    if not ensure_ml_runtime(silent=True):
        return None
...
def _sanitize_candidate_text(raw: str) -> str:
    cleaned = raw.replace("User:", "").replace("AI:", "").replace("Teacher:", "").strip()
    cleaned = " ".join(cleaned.split())
    return cleaned


def _is_bad_fallback_memory(text: str) -> bool:
    lowered = text.lower()
    blocked_fragments = [
        "offline-режиме",
        "внешний провайдер недоступен",
        "провайдер недоступен",
        "вот релевантная мысль",
        "qwen provider",
    ]
    return any(fragment in lowered for fragment in blocked_fragments)


def _offline_direct_reply(user_text: str, user_name: str | None, style: str) -> str:
    base = "Я на связи в offline-режиме и могу помочь с идеями, кодом, обучением и планированием."
    if user_name:
        base = f"{user_name}, я на связи в offline-режиме и могу помочь с идеями, кодом, обучением и планированием."

    if style == "talkative":
        return (
            f"{base} "
            "Если хочешь, разберём вопрос пошагово: сначала цель, потом ограничения, затем конкретный план действий. "
            "Напиши, что именно нужно получить в итоге, и я дам развёрнутый ответ."
        )
    return base
...
def fallback_response(db: CorpusDB, user_text: str, chat_context: list[str], style: str = "balanced") -> str:
    ...
    if any(g in text for g in ["hi", "hello", "привет", "здар", "добрый"]):
        if user_name:
            return f"Привет, {user_name}! Я в offline-режиме, но готов нормально общаться и помогать. Что разбираем?"
        return "Привет! Я в offline-режиме, но готов нормально общаться и помогать. Что разбираем?"

    if "name" in text or "зовут" in text or "имя" in text:
        if user_name:
            return f"Тебя зовут {user_name}. Я это запомнил в текущем диалоге."
        return "Пока не знаю твоё имя. Напиши: 'Меня зовут ...', и я запомню."

    local = db.search(user_text, limit=8)
    for _, candidate in local:
        cleaned = _sanitize_candidate_text(candidate)
        if len(cleaned) < 35:
            continue
        if any(marker in candidate for marker in ["User:", "AI:", "Teacher:"]):
            continue
        if _is_bad_fallback_memory(cleaned):
            continue
        return cleaned

    samples = db.sample_texts(8)
    for candidate in samples:
        ...
        if _is_bad_fallback_memory(cleaned):
            continue
        return f"Сейчас внешний провайдер недоступен. Близкая идея из локальной базы: {cleaned}"

    return _offline_direct_reply(user_text, user_name, style)
...
def cmd_sync_hf(args):
    ...
    for url in HF_DATASET_URLS:
        spinner = AnimatedSpinner("Loading HF dataset rows")
        spinner.start()
        try:
            texts = fetch_hf_rows_as_texts(url, max_rows=args.hf_rows)
        except Exception as exc:
            spinner.stop()
            print_system(f"HF sync failed for source: {url} | reason: {exc}")
            continue
        spinner.stop()
        if texts:
            db.insert_texts(texts)
            added_total += len(texts)
            print_system(f"Loaded {len(texts)} rows from: {url}")
    print(f"[OK] HF sync complete. Added {added_total} rows. Total texts: {db.count_texts()}")
...
def cmd_teacher(args):
    db = CorpusDB(DB_PATH)
    spinner = AnimatedSpinner("Qwen teacher")
    spinner.start()
    try:
...
/sync-hf                загрузить данные с HuggingFace datasets
...
def cmd_chat(args):
    ...
    if not os.getenv("HF_TOKEN"):
        print_system("Подсказка: установи HF_TOKEN для ответов Qwen через HuggingFace Router")
    ...
    while True:
        ...
        if cmd == "/db clear":
            ...
            continue
        if cmd == "/model info":
            ml_runtime = "ok" if ensure_ml_runtime(silent=True) else "unavailable"
            print_system(
        ...
        if cmd == "/init-db":
            ...
            continue
        if cmd == "/sync-hf":
            cmd_sync_hf(argparse.Namespace(hf_rows=100))
            continue
        ...
        try:
            ...
            if not PROVIDER_STATUS_PRINTED:
                print_system(f"Qwen provider недоступен: {exc}")
        ...
        if turn_count % LEARN_EVERY == 0:
            print_system(f"Диалог сохранён в базу ({db.stats()['texts']} текстов). Локальное дообучение: /train")


def build_parser():
    p = argparse.ArgumentParser(description="TokenCode AI v6.3")
    ...
    a_chat.add_argument("--hf-rows", type=int, default=100)
    ...
    a_hf = sub.add_parser("sync-hf")
    a_hf.add_argument("--hf-rows", type=int, default=100)

#!/usr/bin/env python3
"""TokenCode AI v6.2: Local Torch + HuggingFace Router Qwen fallback chat."""
...
def fallback_response(db: CorpusDB, user_text: str, chat_context: list[str], style: str = "balanced") -> str:
    ...
    if any(g in text for g in ["hi", "hello", "привет", "здар", "добрый"]):
        if user_name:
            return (
                f"Привет, {user_name}! Я сейчас в offline-режиме, но остаюсь на связи. "
                "Давай продолжим: могу обсудить идеи, код, обучение модели или просто поболтать."
            )
        return (
            "Привет! Я сейчас в offline-режиме, но могу нормально общаться. "
            "Задай вопрос, и я отвечу максимально полезно."
        )

    if "name" in text or "зовут" in text or "имя" in text:
        if user_name:
            return f"Тебя зовут {user_name}. Я запомнил это в текущем диалоге."
        return "Пока не знаю твоё имя. Напиши: 'Меня зовут ...', и я запомню."

    local = db.search(user_text, limit=5)
    for _, candidate in local:
        cleaned = candidate.replace("User:", "").replace("AI:", "").replace("Teacher:", "").strip()
        if len(cleaned) >= 25:
            return cleaned

    for candidate in db.sample_texts(5):
        cleaned = candidate.replace("User:", "").replace("AI:", "").replace("Teacher:", "").strip()
        if len(cleaned) >= 25:
            return (
                "Сейчас провайдер недоступен, поэтому отвечаю по локальной базе знаний. "
                f"Вот релевантная мысль: {cleaned}"
            )

    if style == "talkative":
        return (
            "Сейчас внешний провайдер недоступен, но диалог продолжается. "
            "Могу помочь структурировать вопрос, предложить план действий и разобрать задачу по шагам. "
            "Напиши тему, и я дам развернутый ответ."
        )
    return "Внешний провайдер недоступен. Сформулируй вопрос чуть подробнее, и я отвечу из локального контекста."
...
def build_parser():
    p = argparse.ArgumentParser(description="TokenCode AI v6.2")
